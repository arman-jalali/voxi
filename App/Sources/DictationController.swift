...
final class DictationController {
    ...
    init() {
        ...
        coordinator = DictationCoordinator(
            audioFactory: { [warmEngines] in
                let engine = warmEngines.take()
                // Live preview rides along on the capture buffers; it owns no
                // audio of its own and cannot delay or fail the recording.
                LivePreview.shared.begin()
                engine.onPCM = { LivePreview.shared.feed($0) }
                return engine
            },
    ...
    private var needsOnboarding: Bool {
        // Local model: no API key to collect — onboarding is permissions only.
        !AXIsProcessTrusted()
    ...
    private func startHistoryServices() {
        ...
        queue.onDrainBlocked = { [weak self] error in
            let message: String
            if case .auth = error {
                message = "Queued dictations are waiting — start the local model to retry them"
            } else {
    ...
    private func bind() {
        // Live preview text → pill. Guarded on state: a late chunk landing after
        // key-up must not repaint a caption over the processing pill.
        LivePreview.shared.onText = { [weak self] text in
            guard let self, case .listening = self.hud.model.state else { return }
    ...
    private static func copy(for failure: DictationFailure) -> String {
        switch failure {
        case .network: return "Local model isn't running — saved to History"
        case .auth: return "Local model isn't reachable — saved to History"
        case .modelAccess: return "Local model isn't available — saved to History"
        case .badRequest: return "The model rejected the request — saved to History"
        case .rateLimited: return "Model busy — History will retry it shortly"
        case .noMicrophone: return "No microphone found — connect one to dictate"
        case .quotaExhausted: return "Model overloaded — saved to History"
        case .timeout: return "Timed out — saved to History"
        case .validation: return "Couldn't transcribe — saved to History"
        case .safetyBlocked: return "The model declined this one — saved to History"
