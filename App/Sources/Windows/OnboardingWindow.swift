...
extension Notification.Name {
    static let onboardingWindowClosed = Notification.Name("com.voxi.app.onboarding.closed")
    static let onboardingJumpToScreen = Notification.Name("com.voxi.app.onboarding.jump")
}
...
private struct OnboardingFlow: View {
    ...
    private var currentScreen: some View {
        ...
        case .apiKey: LocalModelScreen(onNext: { advance() })
...
private struct LocalModelScreen: View {
    let onNext: () -> Void
    @State private var checking = true
    @State private var healthy = false

    var body: some View {
        ScreenScaffold("Runs on your Mac.", "Voxi transcribes with Voxtral, a local model. Your voice never leaves this machine — no account, no API key.") {
            VStack(spacing: JotUI.Spacing.s) {
                if checking {
                    ProgressView().controlSize(.small)
                    Text("Checking the local model…")
                        .font(JotUI.TypeScale.labelSmall())
                        .foregroundStyle(JotUI.Colors.onSurfaceVariant)
                } else if healthy {
                    Label("Voxtral is running and warm", systemImage: "checkmark.circle.fill")
                        .font(JotUI.TypeScale.body())
                        .foregroundStyle(JotUI.Colors.success)
                } else {
                    Label("Model server isn't running yet", systemImage: "exclamationmark.triangle.fill")
                        .font(JotUI.TypeScale.body())
                        .foregroundStyle(JotUI.Colors.error)
                    Text("Voxi will start it automatically — the first launch loads the model and can take a minute.")
                        .font(JotUI.TypeScale.labelSmall())
                        .foregroundStyle(JotUI.Colors.onSurfaceVariant)
                        .multilineTextAlignment(.center)
                        .fixedSize(horizontal: false, vertical: true)
