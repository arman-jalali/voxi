...
struct PillView: View {
    ...
    var body: some View {
        VStack(spacing: JotUI.Spacing.xs) {
            ...
            .animation(JotMotion.emphasizedDecelerate(0.2), value: model.partial)
    }

    /// The words as they are recognised, above the pill. Shown only while
    /// listening — at key-up the pill takes over and the authoritative
    /// transcript goes to the cursor, so a stale caption never lingers.
    @ViewBuilder
    private var liveCaption: some View {
        if case .listening = model.state, !model.partial.isEmpty {
            Text(model.partial)
                .font(JotUI.TypeScale.label())
                .foregroundStyle(JotUI.Colors.onSurface)
                .lineLimit(2)
                .truncationMode(.head) // the newest words are the useful ones
                ...
                .accessibilityHidden(true) // the pill already announces state
        }
    }
...
extension Notification.Name {
    static let pillStopTapped = Notification.Name("com.voxi.app.pill.stop")
    static let pillDotTapped = Notification.Name("com.voxi.app.pill.dot")
}
