...
**Local dictation with [Voxtral-Mini-4B-Realtime](https://huggingface.co/mistralai/Voxtral-Mini-4B-Realtime-2602) (MLX). Hold a key. Speak. It types.**

Dictation for macOS that puts text wherever your cursor is — with the model
running entirely on your Mac. No API key, no account, no audio leaving the machine.

<sub>A local-model fork of [Jot](https://github.com/google-gemini/jot-gemini-transcribe-macOS) by [Ammaar Reshi](https://x.com/ammaar) · Apache 2.0 licensed</sub>
...
Requires Apple Silicon, macOS 14+, Command Line Tools (no Xcode needed), Python 3.11+, and ~4 GB disk for the model.

```bash
./scripts/install-server.sh   # venv + voxmlx + server into ~/Library/Application Support/Voxi
./scripts/build-app.sh        # SwiftPM build + assembles build/Voxi.app
open build/Voxi.app
```

The app starts the Voxtral server automatically at launch (first start downloads
the model and takes a minute; after that it loads in seconds and stays warm).
Transcription runs against `http://127.0.0.1:48765` — nothing leaves the Mac.
...
No Xcode needed — Apple's **Command Line Tools** are enough (`xcode-select --install`).

**Requirements**
- Apple Silicon Mac (the model runs on MLX; Intel is not supported)
- macOS 14 or later
- Python 3.11+ (`brew install python`)
- ~4 GB free disk for the model weights, 16 GB RAM recommended

```bash
./scripts/install-server.sh   # venv + voxmlx + model into ~/Library/Application Support/Voxi
./scripts/build-app.sh        # SwiftPM build, ad-hoc sign, install to /Applications/Voxi.app
./scripts/test.sh             # swift test on JotCore
```

The app is ad-hoc signed, so **every rebuild resets its Accessibility and
Microphone grants** — re-add `/Applications/Voxi.app` in System Settings →
Privacy & Security after installing a new build. Errors you may hit:

| Symptom | Cause |
| --- | --- |
| `No module named 'mlx'` / `ImportError: ... arm64` | Intel Mac or an x86 Python — MLX is Apple Silicon only |
| `python3: command not found` or Python < 3.11 | install with `brew install python` |
| `error: external macro implementation type 'PreviewsMacros...'` | building a dependency with `#Preview` under CLT — `Vendor/KeyboardShortcuts` already has these stripped; don't swap it for the upstream package |
| pill says *Local model isn't running* | run `~/Library/Application\ Support/Voxi/server.sh` and watch its output |
...
JotCore/        all engine logic, headless and testable
  ...
  TranscriptionClient/  local Voxtral client, WAV encode, live preview, timeouts
...
server/         voxi_server.py (batch + streaming HTTP on 127.0.0.1) and stream_session.py
Vendor/         KeyboardShortcuts, vendored with #Preview blocks removed
scripts/        install-server, build-app, test, icon, DMG
docs/           privacy, design specs, research (some upstream docs describe Jot's Xcode flow)
```
