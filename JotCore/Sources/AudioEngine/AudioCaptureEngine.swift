...
public final class AudioCaptureEngine: AudioCapturing {
    ...
    /// Converted 16kHz/mono/Int16 PCM, delivered on the write queue as each
    /// buffer lands. Used by the live-preview transcriber; nil by default so the
    /// recording path costs nothing when preview is off.
    ...
    private let queue = DispatchQueue(label: "com.voxi.app.audio.write", qos: .userInitiated)
    ...
    private func ingest(_ buffer: AVAudioPCMBuffer, hwRate: Double) {
        ...
        queue.async { [weak self] in
            ...
            if let onPCM = self.onPCM, let channel = out.int16ChannelData?[0] {
                onPCM(Data(bytes: channel, count: Int(out.frameLength) * MemoryLayout<Int16>.size))
            }
            do {
