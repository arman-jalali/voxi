...
- Watch traffic with Little Snitch or `nettop` — you'll see **no** remote host. The only
  connection is to `127.0.0.1:48765`, the local Voxtral server started by the app.
- The model weights are fetched once from Hugging Face (`mlx-community/Voxtral-Mini-4B-Realtime-6bit`)
  by `scripts/install-server.sh` / first launch; that is the only download, and it contains no audio.
