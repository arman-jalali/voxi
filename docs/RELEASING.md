> **Voxi fork note:** this document describes the upstream Jot release process (Xcode archive, Developer ID, notarised DMG). Voxi is built with SwiftPM via `scripts/build-app.sh` and is not distributed as a signed DMG. Kept for reference only.
